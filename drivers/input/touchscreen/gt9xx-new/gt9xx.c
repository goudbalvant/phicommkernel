...
#elif defined (CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_FB)
/*******************************************************
Function:
    Enter sleep mode.
Input:
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    GTP_DEBUG_FUNC();
    ret = GTP_GPIO_REQUEST(GTP_INT_PORT, "goodix_ts_irq_gpio");
    if (ret < 0) 
    {
    ...
    ret = GTP_GPIO_REQUEST(GTP_RST_PORT, "goodix_ts_reset_gpio");
    if (ret < 0) 
    {
...
static int goodix_power_on(struct goodix_ts_data *ts)
{
	...
	if (ts->power_on) {
		GTP_INFO("Device already power on\n");
		return 0;
	}
	...
	if (!IS_ERR(ts->avdd)) {
		...
		if (ret < 0) {
			GTP_ERROR("Regulator avdd set_opt failed rc=%d\n", ret);
			goto err_set_opt_avdd;
		}
		ret = regulator_enable(ts->avdd);
		if (ret) {
			GTP_ERROR("Regulator avdd enable failed ret=%d\n", ret);
	...
	if (!IS_ERR(ts->vdd)) {
		ret = regulator_set_voltage(ts->vdd, GOODIX_VTG_MIN_UV,
					   GOODIX_VTG_MAX_UV);
		if (ret) {
			GTP_ERROR("Regulator set_vtg failed vdd ret=%d\n", ret);
			goto err_set_vtg_vdd;
		}
		ret = reg_set_optimum_mode_check(ts->vdd,
			GOODIX_VDD_LOAD_MAX_UA);
		if (ret < 0) {
			GTP_ERROR("Regulator vdd set_opt failed rc=%d\n", ret);
			goto err_set_opt_vdd;
		}
		ret = regulator_enable(ts->vdd);
		if (ret) {
			GTP_ERROR("Regulator vdd enable failed ret=%d\n", ret);
			goto err_enable_vdd;
		}
	}
	...
	if (!IS_ERR(ts->vcc_i2c)) {
		...
		if (ret) {
			GTP_ERROR("Regulator set_vtg failed vcc_i2c ret=%d\n",ret);
			goto err_set_vtg_vcc_i2c;
		}
		ret = reg_set_optimum_mode_check(ts->vcc_i2c,
			GOODIX_VIO_LOAD_MAX_UA);
		if (ret < 0) {
			GTP_ERROR("Regulator vcc_i2c set_opt failed rc=%d\n",ret);
			goto err_set_opt_vcc_i2c;
		}
		ret = regulator_enable(ts->vcc_i2c);
		if (ret) {
			GTP_ERROR("Regulator vcc_i2c enable failed ret=%d\n",ret);
...
static int goodix_power_off(struct goodix_ts_data *ts)
{
	...
	if (!ts->power_on) {
		GTP_INFO("Device already power off\n");
		return 0;
	}
	...
	if (!IS_ERR(ts->vcc_i2c)) {
		ret = regulator_set_voltage(ts->vcc_i2c, 0,
			GOODIX_I2C_VTG_MAX_UV);
		if (ret < 0)
			GTP_ERROR("Regulator vcc_i2c set_vtg failed ret=%d\n",ret);
		ret = regulator_disable(ts->vcc_i2c);
		if (ret)
			GTP_ERROR("Regulator vcc_i2c disable failed ret=%d\n",
				ret);
	}
	...
	if (!IS_ERR(ts->vdd)) {
		...
		if (ret < 0)
			GTP_ERROR("Regulator vdd set_vtg failed ret=%d\n", ret);
		ret = regulator_disable(ts->vdd);
		if (ret)
			GTP_ERROR("Regulator vdd disable failed ret=%d\n", ret);
	}

	if (!IS_ERR(ts->avdd)) {
		ret = regulator_disable(ts->avdd);
		if (ret)
			GTP_ERROR("Regulator avdd disable failed ret=%d\n", ret);
	}
...
static int goodix_power_init(struct goodix_ts_data *ts)
{
	...
	if (IS_ERR(ts->avdd)) {
		ret = PTR_ERR(ts->avdd);
		GTP_INFO("Regulator get failed avdd ret=%d\n", ret);
	}

	ts->vdd = regulator_get(&ts->client->dev, "vdd");
	if (IS_ERR(ts->vdd)) {
		ret = PTR_ERR(ts->vdd);
		GTP_INFO("Regulator get failed vdd ret=%d\n", ret);
	}

	ts->vcc_i2c = regulator_get(&ts->client->dev, "vcc-i2c");
	if (IS_ERR(ts->vcc_i2c)) {
		ret = PTR_ERR(ts->vcc_i2c);
		GTP_INFO("Regulator get failed vcc_i2c ret=%d\n", ret);
	}
				...
				struct goodix_ts_platform_data *pdata)
{
	...
	if (!strcmp(name, "goodix,panel-coords")) {
		...
		GTP_ERROR("unsupported property %s\n", name);
		return -EINVAL;
	}
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	rc = of_property_read_string(np, "goodix,product-id",
						&pdata->product_id);
	if (rc && (rc != -EINVAL)) {
		GTP_ERROR("Failed to parse product_id.");
		return -EINVAL;
	}
	...
	rc = of_property_read_string(np, "goodix,fw_name",
						&pdata->fw_name);
	if (rc && (rc != -EINVAL)) {
		GTP_ERROR("Failed to parse firmware name.\n");
		return -EINVAL;
	}
	...
	if (prop) {
		...
		rc = of_property_read_u32_array(np,
			...
			num_buttons);
		if (rc) {
			GTP_ERROR("Unable to read key codes\n");
			return rc;
		}
	...
	for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++) {
		...
		if (!pdata->config_data[i]) {
			GTP_ERROR("Not enough memory for panel config data %d\n",i);
			return -ENOMEM;
		}
...
static int goodix_ts_suspend(struct device *dev)
{
	...
	if (ts->fw_loading) {
		GTP_INFO("Fw upgrade in progress, can't go to suspend.");
