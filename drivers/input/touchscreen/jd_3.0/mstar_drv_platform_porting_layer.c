...
static unsigned int MS_TS_MSG_IC_GPIO_RST = 12;
static unsigned int MS_TS_MSG_IC_GPIO_INT = 13;
static unsigned int MSG_INT_GPIO_FLAG = 0;
static unsigned int MSG_RESET_GPIO_FLAG = 0;
...
//extern struct regulator *g_ReguVcc;
...
s32 DrvPlatformLyrTouchDeviceRequestGPIO(struct device *dev)
{
    s32 nRetVal = 0;
...
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
	MS_TS_MSG_IC_GPIO_RST=of_get_named_gpio_flags(np, "msg21xxA,reset-gpio",
                                0, &MSG_RESET_GPIO_FLAG);
    ...
    MS_TS_MSG_IC_GPIO_INT = of_get_named_gpio_flags(np, "msg21XXA,irq-gpio", 0, &MSG_INT_GPIO_FLAG);
