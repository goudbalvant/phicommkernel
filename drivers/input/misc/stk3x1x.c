...
#define STK_LT_DEF     6000	//1600
#define STK_HT_DEF     6200	//1650
#define STK_LT_PT      1000	//400
#define STK_HT_PT      1100	//450
				...
				unsigned int delay_msec)
{
	...
	if(als_data==NULL){
		printk("als_poll_delay_set get data error\n");
		return 0;
	}
	mutex_lock(&als_data->io_lock);
	if(delay_msec<5){
		als_data->als_poll_delay=ns_to_ktime(5 * NSEC_PER_MSEC);;
	}
	else {
		als_data->als_poll_delay = ns_to_ktime(delay_msec * NSEC_PER_MSEC);
	}
	mutex_unlock(&als_data->io_lock);
//	printk("pengwei-----als_poll_delay_set: %d\n",als_data->als_poll_delay);
	return 0;
...
#ifdef CONFIG_DEVICE_VERSION
        confirm_fdv(DEV_L_P,MANUF_STK,MANUF_STK_ID|0x48);
#endif
	return 0;
...
static int __init stk3x1x_init(void)
{
	int ret;
#ifdef CONFIG_DEVICE_VERSION
        register_fdv_with_desc(DEV_L_P,MANUF_STK,MANUF_STK_ID|0x48,DEVICE_NAME);
#endif
